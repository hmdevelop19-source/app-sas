...
import { Loader2, ArrowLeft, Save, FileText, AlertCircle, Calendar } from 'lucide-react';
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-slate-500">Memuat profil siswa...</p>
      </div>
    );
  }

  if (!student) {
    return <div className="p-6 text-center text-rose-500">Data siswa tidak ditemukan.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/students')} className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-colors">
          <ArrowLeft size={20} />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{student.name}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">NIS: {student.nis} • Siswa Aktif</p>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-700">
        ...
        <button 
          ...
          className={`px-6 py-3 font-medium text-sm border-b-2 transition-colors ${activeTab === 'riwayat' ? 'border-sky-500 text-sky-600 dark:text-sky-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
        >
          Riwayat SP & Pelanggaran
        </button>
      </div>
      ...
      {activeTab === 'profil' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 space-y-6 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">NIK Siswa</label>
          ...
          <Card className="p-6 space-y-6 bg-slate-50/50 dark:bg-slate-800/20">
            <h3 className="text-lg font-semibold border-b border-slate-200 dark:border-slate-700 pb-3">Informasi Wali</h3>
            {student.guardian ? (
              <div className="space-y-4">
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Nama Wali ({student.guardian.relationship || 'Wali'})</p>
                  <p className="font-medium text-slate-800 dark:text-slate-200">{student.guardian.name}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Nomor Telepon</p>
                  <p className="font-medium text-slate-800 dark:text-slate-200">{student.guardian.phone || '-'}</p>
                </div>
                <div className="pt-4">
                  <button onClick={() => navigate(`/guardians/${student.guardian.id}`)} className="w-full py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sky-600 hover:bg-slate-50 transition-colors text-sm font-medium">
                    Lihat Profil Wali
                  </button>
                </div>
              </div>
            ) : (
              <p className="text-sm text-slate-500">Belum ada wali yang terhubung.</p>
            )}
          </Card>
        </div>
      ) : (
        <Card className="p-0 overflow-hidden">
          <div className="p-6 text-center text-slate-500">
            ...
            <h3 className="text-lg font-medium text-slate-700 dark:text-slate-200">Belum Ada Riwayat SP</h3>
            <p className="text-sm mt-1">Siswa ini belum pernah menerima Surat Peringatan.</p>
          </div>
        </Card>
      )}
    </div>
